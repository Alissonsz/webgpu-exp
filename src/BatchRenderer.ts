...
export class BatchRenderer {
  ...
  private static initIndexBuffer() {
    ...
    BatchRenderer.device.queue.writeBuffer(BatchRenderer.indexBuffer, 0, BatchRenderer.indexBufferData.buffer);
  }
  ...
  static begin(camera: Camera) {
    ...
    BatchRenderer.device.queue.writeBuffer(BatchRenderer.viewMatrixBuffer, 0, camera.getViewProjectionMatrix().buffer);
  }
  ...
  private static flush() {
    ...
    const TEXTURE_BINDS_START = bindGroupEntries.length;
    BatchRenderer.texturesUsedThisBatch.forEach((t, i) =>
      bindGroupEntries.push({
        binding: i + TEXTURE_BINDS_START,
        resource: t,
      }),
    ...
    BatchRenderer.device.queue.writeBuffer(
      BatchRenderer.vertexBuffer,
      0,
      BatchRenderer.vertexBufferData.buffer,
      0,
      BatchRenderer.pendingQuads * VertexData.F32_LENGTH * VERTICES_PER_QUAD * 4,
    );
